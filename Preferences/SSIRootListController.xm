...
- (instancetype)init
{
    ...
    if (self)
	{
        SSIAppearanceSettings *appearanceSettings = [[SSIAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"SunriseSunsetInfo";
...
{
    if (scrollView.contentOffset.y > [SSIRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"SunriseSunsetInfo"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.sunrisesunsetinfoprefs"] removeAllObjects];
